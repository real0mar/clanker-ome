...
const TELEGRAM_API_BASE = TELEGRAM_TOKEN
...
const SPOTIFY_REGEX =
	/(https?:\/\/(?:[a-z]+\.)?spotify\.com\/[^\s]+|https?:\/\/spotify\.link\/[^\s]+)/gi;
		...
		try {
			...
			if (!metadata) {
				await sendTelegramMessage(
					chatId,
					`${announcer} shared a Spotify link but I could not load its details.`,
					replyToMessageId,
				);
				continue;
			}
			...
			await sendTelegramMessage(
				chatId,
				`${announcer} shared a Spotify link but something went wrong while expanding it.`,
				replyToMessageId,
			);
		}
	}
	...
	switch (type) {
		case "track": {
			...
			const externalUrl: string =
				...
				`https://open.spotify.com/track/${idFallback}`;
			return {
				type,
				title,
				subtitle: artists,
		...
		case "album": {
			...
			const externalUrl: string =
				data.external_urls?.spotify ??
				`https://open.spotify.com/album/${idFallback}`;
			return {
				type,
				title,
				subtitle: artists,
		...
		case "playlist": {
			const title: string = data.name;
			const owner: string =
				data.owner?.display_name ?? data.owner?.id ?? "unknown curator";
			const image = data.images?.[0]?.url;
			const externalUrl: string =
				data.external_urls?.spotify ??
				`https://open.spotify.com/playlist/${idFallback}`;
			return {
				type,
				title,
				subtitle: owner,
		...
		case "artist": {
			...
			const externalUrl: string =
				data.external_urls?.spotify ??
				`https://open.spotify.com/artist/${idFallback}`;
			return {
				type,
				title,
				subtitle: genres,
		...
		case "show": {
			...
			const externalUrl: string =
				data.external_urls?.spotify ??
				`https://open.spotify.com/show/${idFallback}`;
			return {
				type,
				title,
				subtitle: publisher,
		...
		case "episode": {
			...
			const year = deriveReleaseYear(
				data.release_date ?? data.release_date_time,
			);
			const image = data.images?.[0]?.url ?? data.show?.images?.[0]?.url;
			const externalUrl: string =
				data.external_urls?.spotify ??
				`https://open.spotify.com/episode/${idFallback}`;
			return {
				type,
				title,
				subtitle: showName,
...
async function callTelegramApi<T extends Record<string, unknown>>(
	method: string,
	body: T,
) {
	if (!TELEGRAM_API_BASE) {
...
function formatCaption(metadata: SpotifyMetadata, announcer: string): string {
	...
	switch (metadata.type) {
		case "track":
		case "album": {
			const yearSuffix = metadata.releaseYear
